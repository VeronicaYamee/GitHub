...
The time here is represented instead of using seconds, using ticks. A tick is an arbitrary unit for measuring system time. The number of ticks per second (the tick rate) is the frequency of the system timer which is measured in hertz. One second corresponds to ten million ticks. Therefore, if you want to find the frequency value, you need just to divide the number of ticks by the time in seconds.
