...
The time here is represented by ticks instead of seconds. A tick is an arbitrary unit for measuring system time. The number of ticks per second (the tick rate) is the frequency of the system. Therefore, if you want to find the number of ticks, you just need to multiply the frequency value in hertz by the time in seconds. 
